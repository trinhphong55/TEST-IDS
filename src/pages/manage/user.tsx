  ...
  const getDataAPi = async () => {
    try {
      const response = await axios.get(
        'https://651408878e505cebc2ea954e.mockapi.io/getUser'
      )
  ...
  const getDataSearch = (value: User, index: number) => {
    if (!value?.id && !value?.username && !value?.role) {
      ...
      var newSearch = data?.filter((item: User) => {
        if (value?.role && !value?.id && !value?.username) {
    ...
    if (user == null) {
      ...
      try {
        const response = await axios.put(
          `https://651408878e505cebc2ea954e.mockapi.io/getUser/${value.id}`,
  ...
  const handleDelete = async (value: string) => {
    try {
      const response = await axios.delete(
        `https://651408878e505cebc2ea954e.mockapi.io/getUser/${value}`
      )
      if (response?.statusText == 'OK') {
        message.success('success')
      } else {
